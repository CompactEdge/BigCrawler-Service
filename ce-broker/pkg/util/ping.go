/*
	from "github.com/sparrc/go-ping"
*/
...
// Stop is member of util package
func (p *Pinger) Stop() {
	close(p.done)
}
...
func (p *Pinger) recvICMP(
	...
	for {
		...
		case <-p.done:
			return
		default:
			byteData := make([]byte, 512)
			_ = conn.SetReadDeadline(time.Now().Add(time.Millisecond * 100))
			var n, ttl int
			var err error
			if p.ipv4 {
				var cm *ipv4.ControlMessage
				n, cm, _, err = conn.IPv4PacketConn().ReadFrom(byteData)
				if cm != nil {
					ttl = cm.TTL
				}
			} else {
				var cm *ipv6.ControlMessage
				n, cm, _, err = conn.IPv6PacketConn().ReadFrom(byteData)
				if cm != nil {
					ttl = cm.HopLimit
				}
			}
			...
			recv <- &packet{bytes: byteData, nbytes: n, ttl: ttl}
		}
	}
}
